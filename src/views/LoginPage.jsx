...
function SensorTicker() {
  ...
  return (
    <div className="login-ticker">
      ...
      <div className="login-ticker-bar"><div className="login-ticker-bar-fill" key={i}></div></div>
    </div>
  );
}
...
export default function LoginPage({ onBack }) {
  ...
  // One login for everyone — the account's own record decides whether they
  // land in the admin console or the user dashboard. An admin record wins
  // if an email happens to hold both (matches the previous tab default).
  ...
  return (
    <div className="login-screen">
      <div className="login-panel-brand">
        ...
        <div className="login-brand-byline">by JustEmbedded · a Susima Smaart Solutions company</div>
        <div className="login-brand-sub">IoT sensor monitoring for cold storage, offices, warehouses and everything in between.</div>
        <div className="login-brand-points">
          {BRAND_POINTS.map(p => (
            <div className="login-brand-point" key={p.text}><svg><use href={`#${p.icon}`} /></svg>{p.text}</div>
          ))}
        </div>
        <SensorTicker />
      </div>
      ...
      <div className="login-panel-form">
        {onBack && (
          <button type="button" className="login-back" onClick={onBack}>
            <svg style={{ width: 13, height: 13, transform: 'rotate(180deg)' }}><use href="#i-chev" /></svg>Back to home
          </button>
        )}

        <div className="login-title">Sign in to JustEdge</div>
        <div className="login-sub">Manage your fleet, or view the sensors assigned to you</div>
